 ...
 ╰──────•◈•──────╯`,MessageType.text);
    ...
    Asiata.addcommand({ pattern: 'insta ?(.*)', fromMe: true,  deleteCommand: false, desc: Lang.IG_DESC}, (async (message, match) => {
      ...
      if (msg === '200') {
        ...
        await message.sendMessage(Buffer.from(profileBuffer.data), MessageType.video, {caption: " ┋⚔️𝐒𝐈𝐌𝐏𝐋𝐄-𝐁𝐎𝐓⚔️┋"}) 
        }
          })
          .catch(
...
╚╩══• •✠•❀•✠ • •══╩╝`,MessageType.text, {quoted: message.data});
	...
	Asiata.addcommand({pattern: 'video ?(.*)', fromMe: false, desc: Lang.VIDEO_DESC}, (async (message, match) => { 
        ...
        yt.on('end', async () => {
            ...
            await message.client.sendMessage(message.jid,fs.readFileSync('./' + VID + '.mp4'), MessageType.video,  { caption: '⚔️𝐒𝐈𝐌𝐏𝐋𝐄-𝐁𝐎𝐓⚔️' });
        });
    }));
    ...
    Asiata.addcommand({ pattern: 'insta ?(.*)', fromMe: false, desc: Lang.IG_DESC}, (async (message, match) => {
      ...
      if (msg === '200') {
        ...
        await message.sendMessage(Buffer.from(profileBuffer.data), MessageType.video, {quoted: message.data}, {caption: "┋⚔️𝐒𝐈𝐌𝐏𝐋𝐄-𝐁𝐎𝐓⚔️┋"}) 
        }
          })
          .catch(
